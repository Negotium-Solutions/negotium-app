...
onMounted(() => {
  profileDetailStore.set('profileTypes', props.profileTypes);
    ...
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="row ml-2 mb-2">
          <div class="px-1 mb-1" v-for="(profileType, index) in profileDetailStore.profileTypes" :key="index">
            <a :href="route('profile.create', { schema_id: profileType.id })" class="float-right h-[38px] p-3 bg-white rounded border border-neutral-700 justify-center items-center gap-2 inline-flex ml-1 text-xs font-bold cursor-pointer" :class="{ 'bg-zinc-100 text-yellow-100' : profileType.id === props.schema_id }">{{ profileType.name }}</a>
          </div>
        </div>
        <div v-if="profileDetailStore.isSet('profile', profileDetailStore.profile)" class="d-flex flex-column w-100 pl-3 pr-3">

          <div v-for="(profileDetail, index) in profileDetailStore.profile.groups" :key="index" class="flex flex-col px-2 py-2 rounded-lg border border-solid border-neutral-700 mb-2">
            ...
            <div class="row">
              <div v-for="(field, _index) in profileDetail.fields" :key="_index" class="col-md-6 pl-3" :class="{ 'border-top-0 border-bottom-0 border-left-0 border-right-2 border border-solid border-neutral-700 border-opacity-20': _index % 2 === 0 }">
                <div class="gap-28 mb-1">
                  ...
                  <input v-if="field.dynamic_model_field_type_id == 1" class="form-control" v-model="profileDetailStore.profile.groups[index].fields[_index].value">
                  <select class="form-control" v-if="field.dynamic_model_field_type_id == 9" v-model="profileDetailStore.profile.groups[index].fields[_index].value" placeholder="Select a Gender">
