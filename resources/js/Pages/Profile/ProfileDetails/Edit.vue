...
onMounted(() => {
  ...
  console.log('profileDetailStore.form', profileDetailStore.form);
});
</script>
  ...
  <div v-if="profileManagerStore.isSelected('profile', profileManagerStore.profile)" class="col-lg-12 pl-0 pr-0">
    <div class="d-flex w-100 pt-3 pl-3 pr-3 pb-4">
      ...
      <div class="col-sm-6 text-right">
        <a :href="route('profile.profile-details', {id: profileManagerStore.profile.id})+'?s_id='+profileManagerStore.schemaId" class="gap-2 justify-center py-2 px-4 rounded-custom-25 border border-solid border-neutral-700 border-opacity-20 text-neutral-700 hover:bg-neutral-700 hover:text-white mr-2">Cancel</a>
        <button v-if="!profileDetailStore.loading" @click="profileDetailStore.updateProfile(profileManagerStore.profile.id, toast)" class="gap-2 justify-center py-2 px-4 rounded-custom-25 border border-solid border-neutral-700 border-opacity-20 text-neutral-700 hover:bg-neutral-700 hover:text-white">Save</button>
    ...
    <div class="d-flex flex-column w-100 pl-3 pr-3">
      ...
      <div v-for="(profileDetail, index) in profileDetailStore.form.groups" :key="index" class="flex flex-col px-2 py-2 rounded-lg border border-solid border-neutral-700 mb-2">
        ...
        <div class="row">
          <div v-for="(field, _index) in profileDetail.fields" :key="_index" class="col-md-6 pl-3" :class="{ 'border-top-0 border-bottom-0 border-left-0 border-right-2 border border-solid border-neutral-700 border-opacity-20': _index % 2 === 0 }">
            <div class="gap-28 mb-1">
              ...
              <input v-if="field.dynamic_model_field_type_id == 1" class="form-control" v-model="profileDetailStore.form.groups[index].fields[_index].value">
              <select class="form-control" v-if="field.dynamic_model_field_type_id == 9" v-model="profileDetailStore.form.groups[index].fields[_index].value" placeholder="Select a Gender">
                <option v-for="(option, index) in field.options" :key="index">{{ option.name }}</option>
              </select>
              <div class="input-validation-error" v-if="typeof profileDetailStore.formErrors?.[field.field] !== 'undefined'">
                <span v-for="(error, index) in profileDetailStore.formErrors?.[field.field]" :key="index" class="error invalid-feedback">{{ error }}</span>
              </div>
            </div>
          </div>
        </div>
