    ...
    <div class="card card-default">
      ...
      <div class="card-body">
        <div class="mb-2">
          ...
          <p class="mt-1 text-neutral-700 text-sm font-semibold font-['Roboto'] leading-tight">{{ processStore.process.name }}</p>
        </div>
        <div class="mb-1">
          <div class="opacity-50 text-neutral-700 text-xs font-normal font-['Nunito'] leading-3">Category</div>
          <p class="mt-1 text-neutral-700 text-sm font-semibold font-['Roboto'] leading-tight d-inline-block">{{ processStore.process.category.name }}<div class="ml-1 pt-2 w-3 h-3 rounded-[3px] d-inline-block" :style="'background:'+processStore.process.category.color"></div></p>
        </div>
        <div class="mt-3">
        ...
        <div>
          <button type="button" @click="setStep(step)" v-for="(step, index) in processStore.process.steps" :key="index" class="w-100 h-9 p-2 rounded border border-neutral-700/opacity-25 flex-col justify-start items-start gap-2 inline-flex mb-1"  :class="{ 'bg-zinc-100' : stepStore.step.id === step.id }">
            <div :class="stepStore.step.id === step.id ? 'font-medium' : 'font-medium'" class="text-neutral-700 text-sm font-['Roboto'] leading-tight" >{{ step.name }}</div>
          </button>
        </div>
      </div>
      <!-- /.card-body -->
